...
async function download_music(query) {
  return new Promise(async (resolve, reject) => {
    try {
      const searchResults = await searchYouTube(query);
      if (searchResults.length === 0) {
...
module.exports = {igdownloader, twitterScrape, downloadFile, ytMP3, ytMP4, playlagu, fbDownloader, download_music, downloadVoice}
